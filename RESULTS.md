...
The pipeline overview figure in the **paper** repository is `fig/overview.pdf` (or `fig/overview.png`), generated by `Paper/fig/gen_overview_fig.py`. This **Code** package does not ship the LaTeX tree; use Overleaf or a full TeX installation to build the paper (if a package such as `algorithmicx` is missing locally, install it or use the Overleaf project).
